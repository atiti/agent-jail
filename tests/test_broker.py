...
class BrokerTests(unittest.TestCase):
    ...
    def test_claude_bypass_flag_is_allowed_without_jit(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = PolicyStore(os.path.join(tmp, "policy.json"))
            broker = BrokerServer(
                os.path.join(tmp, "broker.sock"),
            ...
            result = broker.handle(
                {
                    ...
                    "raw": "claude --allow-dangerously-skip-permissions",
                    "cwd": tmp,
                }
            )
        self.assertEqual(result["decision"], "allow")

    def test_claude_node_shim_bypass_flag_is_allowed_without_jit(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = PolicyStore(os.path.join(tmp, "policy.json"))
            broker = BrokerServer(
                os.path.join(tmp, "broker.sock"),
            ...
            result = broker.handle(
                {
                    ...
                    "raw": "node /Users/example/.nvm/versions/node/v22.22.0/lib/node_modules/@anthropic-ai/claude-code/cli.js --allow-dangerously-skip-permissions",
                    "cwd": tmp,
                }
            )
        self.assertEqual(result["decision"], "allow")
