import os
import subprocess
import unittest


ROOT = os.path.dirname(os.path.dirname(__file__))


class ManualSuiteTests(unittest.TestCase):
    def test_manual_policy_suite_lists_cases(self):
        proc = subprocess.run(
            ["bash", "scripts/manual_policy_suite.sh", "--list"],
            cwd=ROOT,
            text=True,
            capture_output=True,
        )
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertIn("allow_repo_cat", proc.stdout)
        self.assertIn("deny_system_cat", proc.stdout)
        self.assertIn("deny_python_system_var_read", proc.stdout)
        self.assertIn("deny_ruby_system_read", proc.stdout)
        self.assertIn("deny_shell_var_indirection", proc.stdout)
        self.assertIn("observe_dmesg", proc.stdout)
        self.assertIn("jit_python_auto_allow", proc.stdout)
        self.assertIn("live_python_semantic", proc.stdout)
